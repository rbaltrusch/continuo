...
def find_missing_note(layer, scale_len):
    """returns note (double) with the highest consonance"""
    consonances = []
    for note in range(scale_len):
        consonances.append(
            sum([get_consonance(note_, note) for note_ in layer]) / (len(layer))
        )
    return consonances.index(max(consonances))


def find_min_change_in_smoothness(
    previous_notes, posterior_notes, missing_note, scale_len
):
    """returns index of the layer in which the missing note would create the
    minimum change in smoothness of voice leading"""
    diff = [
        abs(note - missing_note) % scale_len
        for note in previous_notes + posterior_notes
    ]
...
def make_motif(layers, sophistication, scale_len, random_intervals, motif_length):
    ...
    for i in range(motif_length - 1):
        ...
        for _ in range(sophistication):
            ...
            consonances = [
                get_consonance(layer[i - motif_length], tentative_note)
                for layer in layers
            ]
            tentative_consonance = sum(consonances) / (len(layers)) if layers else 0
            notes.append((tentative_note, tentative_consonance))
        most_consonant_note, _ = max(notes, key=lambda x: x[1])
        motif.append(most_consonant_note)
    return motif
...
def decide_on_motif(motif_list, time_length, number_of_layers):
    ...
    if motif_list:
        ...
        possibilities = [False for x in range(poss_new_motifs)] + [
            True for x, _ in enumerate(motif_list)
        ]
